...
$sql_courses = "SELECT course_id, course_title, course_title_a, course_duration, course_uod, week FROM courses ORDER BY course_title";
...
<form method="POST">
    ...
    <div class="input-group">
        ...
        <select name="course_id" id="course_id" class="form-control" required>
            ...
            <?php while ($row = $result_courses->fetch_assoc()) { ?>
                <option 
                    value="<?= $row['course_id'] ?>"
                    data-duration="<?= htmlspecialchars($row['course_duration'] . ' ' . $row['course_uod']) ?>">
    ...
    <div class="input-group">
        ...
        <select name="cat_id" id="cat_id" class="form-control" required>
            ...
            <?php while ($row = $result_categories->fetch_assoc()) { ?>
                <option value="<?= $row['cat_id'] ?>"><?= htmlspecialchars($row['cat_name']) ?></option>
            <?php } ?>
        </select>
    </div>
    ...
    <div class="input-group">
        <label for="duration">Duration:</label><div style="color:red; font-size:0.8rem">[Input only the NUMBER of Working/Training days, E.g. 1 week = input 5,... 2 weeks = input 10, ...]</div>
        <input type="number" name="duration" id="duration" placeholder="Only NUMBER of ACTUAL TRAINING Days E.g. 5, 10, 130, 80, ...." class="form-control" required>
    </div>
